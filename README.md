...
For simplicity we consider only a single grid row. In the top illustration we note two issues: (1) on the left-hand side (B<sub>0</sub>) we have issues with dual contouring; (2) grid cell centers inside the shape must have negative distance values. First, dual contouring happens because the FMM considers positive values to be outside the shape, i.e. while propagating the FMM computes new values under the assumption that propagation moves away from the interface. Propagated values are always larger than boundary values. Hence, when the algorithm  
